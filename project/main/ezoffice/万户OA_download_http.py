...
def main(target_url):
    ...
    success_msg = now_time() + '[SUCCESS]万户OA downloadhttp.jsp 任意文件下载漏洞存在'
    shell_msg = now_time() + '{}'.format(exp_url)
    warning_msg = now_time() + '[WARNING]万户OA downloadhttp.jsp 任意文件下载漏洞不存在'
    error_msg = now_time() + '[万户OA downloadhttp.jsp 任意文件下载漏洞][ERROR]无法利用poc请求目标或被目标拒绝请求'
    try:
