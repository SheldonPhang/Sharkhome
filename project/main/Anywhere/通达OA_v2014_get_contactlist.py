...
def main(target_url):
    ...
    success_msg = now_time() + '[SUCCESS]  存在通达OA v2014 get_contactlist敏感信息泄漏漏洞'
    shell_msg = now_time() + '{}'.format(exp_url)
    warning_msg = now_time() + '[WARNING]  不存在通达OA v2014 get_contactlist敏感信息泄漏漏洞'
    error_msg = now_time() + '[通达OA v2014 get_contactlist敏感信息泄漏漏洞]请求失败，可能无法与目标建立连接或目标不存在或被拒绝访问'
    try:
