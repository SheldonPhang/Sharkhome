...
------WebKitFormBoundarymVk33liI64J7GQaK'''
    ...
    success_msg = now_time() + '[SUCCESS]泛微 OA uploaderOperate.jsp 文件上传漏洞存在详细https://tvd.wuthreat.com/#/listDetail?TVDID=TVD-2022-15578'
    warning_msg = now_time() + '[WARNING]泛微2022 OA uploaderOperate.jsp 文件上传漏洞不存在'
    error_msg = now_time() + '[泛微2022 OA uploaderOperate.jsp 文件上传漏洞][ERROR]目标连接超时，或者目标不存在'
    try:
